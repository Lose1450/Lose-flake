...

{
  ...
  programs.bash = {
    ...
    shellAliases = {
      ...
      rebuild-nixos = "sudo nixos-rebuild switch --flake /etc/nixos#Remielle && sudo cp /etc/nixos/* ~/Remielle && cd ~/Remielle && git pull lose master && git add . && git commit && git push lose master";
